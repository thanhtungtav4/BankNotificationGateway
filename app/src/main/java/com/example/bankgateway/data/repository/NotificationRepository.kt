...
class NotificationRepository(
    ...
    suspend fun sendOrQueue(config: DeviceConfig, packageName: String, title: String?, text: String?, rawBody: String) {
        if (config.serverUrl == null || config.deviceId == null || config.deviceSecret == null) {
            queue(rawBody, packageName, title, text, "Device is not paired")
            return
        }

        try {
            val request = mobileApi.signedPostRequest(config.serverUrl, "/api/v1/mobile/notifications", config.deviceId, config.deviceSecret, rawBody)
